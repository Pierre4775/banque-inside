    ...
    <div style={{padding:'16px', maxWidth:'700px', margin:'0 auto'}}>
      ...
      <div style={{display:'grid', gridTemplateColumns:'repeat(auto-fit, minmax(200px, 1fr))', gap:'12px', marginBottom:'16px'}}>
        ...
        <div style={{background:'#f0fdf4', borderRadius:'12px', padding:'16px'}}>
          <div style={{fontSize:'12px', color:'#16a34a', marginBottom:'4px'}}>Patrimoine immobilier net</div>
      ...
      <div style={cardStyle}>
        ...
        {biens.map((bien, i) => {
          ...
          const capital = bien.creditLie !== '' && creditsImmo[parseInt(bien.creditLie)]
            ? parseFloat(creditsImmo[parseInt(bien.creditLie)].capital) || 0
            : 0
          return (
              ...
              <div style={{display:'flex', flexDirection:'column', gap:'10px'}}>
                  ...
                  <select style={selectStyle} value={bien.creditLie} onChange={e => updateListe(biens, setBiens, i, 'creditLie', e.target.value)}>
                    ...
                    {creditsImmo.map((c, idx) => (
                      <option key={idx} value={idx}>
                        Credit {idx+1} — {(parseFloat(c.mensualite)||0).toLocaleString()} EUR/mois — Capital restant : {(parseFloat(c.capital)||0).toLocaleString()} EUR
                      </option>
                    ))}
                  </select>
                </div>
                <div style={{background: valeurNette >= 0 ? '#f0fdf4' : '#fef2f2', borderRadius:'8px', padding:'10px'}}>
                  <div style={{fontSize:'12px', color:'#6b7280', marginBottom:'4px'}}>
                    {(parseFloat(bien.valeur)||0).toLocaleString()} EUR {capital > 0 ? `- ${capital.toLocaleString()} EUR (credit)` : ''} = Valeur nette
                  </div>
